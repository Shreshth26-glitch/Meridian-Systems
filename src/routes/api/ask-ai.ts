      ...
      POST: async ({ request }) => {
        ...
        try {
          ...
          if (!lastMessage || !lastMessage.content || lastMessage.content.length > 1000) {
            return new Response(
              JSON.stringify({
                error: "Message content must be present and under 1000 characters.",
              }),
              {
                status: 400,
                headers: { "Content-Type": "application/json" },
...
4. If the user expresses buying intent (e.g. asking about pricing, asking for custom timelines on their project, wanting to talk to someone/get a quote), encourage them to provide their contact information in the lead form that will appear in this chat panel or suggest visiting our contact page.`;
          ...
          if (isMockKey) {
            ...
            if (
              ...
              reply =
                "Yes, we build native iOS apps (10–16 weeks using Swift/SwiftUI) and native Android apps (10–16 weeks using Kotlin/Jetpack Compose). We also build Cross-Platform apps (10–18 weeks using React Native). Would you like to connect with our team to discuss your project?";
            } else if (query.includes("erp") || query.includes("enterprise")) {
              reply =
                "We offer Enterprise ERP Solutions (timeline: 12–20 weeks, using SAP, Odoo, custom REST APIs, and PostgreSQL). This includes modules configuration, data migration, and requirements mapping. Let me know if you would like a scoping call.";
            } else if (
              ...
              reply =
                "I don't have specific pricing details since pricing depends on project scope, but I can connect you with our team for a custom quote. Please provide your email and project details in the form that will appear next.";
            } else if (
              ...
              if (service) {
                ...
                reply =
                  "Typical timelines range from 6 to 12 weeks for websites to 12 to 20 weeks for Enterprise ERP systems. What kind of project are you looking to build?";
              }
            } else if (service) {
              ...
              reply =
                "Meridian Systems offers custom enterprise software development, including Cloud DevOps, ERP systems, Web Applications, Database Architecture, and AI Automations. Let me know if you want to know more about a specific service or request a consultation.";
            }
          ...
          if (!response.ok) {
            ...
            return new Response(
              JSON.stringify({ error: "Failed to communicate with AI provider" }),
              {
                status: 502,
                headers: { "Content-Type": "application/json" },
              },
          ...
          const text =
            data.content?.[0]?.text || "I was unable to generate a response. Please try again.";
