    ...
    <section className="shell py-24 scroll-reveal">
      ...
      <div className="mt-14 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {signatureServices.map((service, i) => (
          <Reveal key={service.slug} delay={i * 80} className="flex h-full w-full">
            <Link
              to="/services/$slug"
              params={{ slug: service.slug }}
              className="signature-card group relative flex flex-col overflow-hidden rounded-xl border border-border bg-background focus-visible:outline-none w-full h-full"
            >
